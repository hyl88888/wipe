...
function Wipe(obj){
	...
	this.transpercent = obj.transpercent;//用户定义百分比
	this.t=0;
...
Wipe.prototype.click = function(){
	...
	that.canvas.addEventListener(that.fn2,function(evt){
		...
		if(that.isMouseDown){
			...
			var X = that.device ? event.touches[0].clientX-getAllLeft(this)+sLeft : event.clientX-getAllLeft(this)+sLeft;
			var Y = that.device ? event.touches[0].clientY-getAllTop(this)+sTop : event.clientY-getAllTop(this)+sTop;
